...
def handler(event, context):
    try:
        ...
        if request_type == 'Create': return associate_vpc_to_hosted_zone(vpc_ids, phz_id)
        if request_type == 'Update': return associate_vpc_to_hosted_zone(vpc_ids, phz_id)
        if request_type == 'Delete': return disassociate_vpc_from_hosted_zone(vpc_ids, phz_id)
